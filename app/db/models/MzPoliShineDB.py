...
class AlarmTable(Base):
    ...
    alarm_type: Mapped[Optional[str]] = mapped_column(String(50, 'utf8mb4_unicode_ci'), comment='报警类型')
    alarm_code: Mapped[Optional[str]] = mapped_column(String(50, 'utf8mb4_unicode_ci'), comment='报警代码')
    alarm_message: Mapped[Optional[str]] = mapped_column(String(255, 'utf8mb4_unicode_ci'), comment='报警信息')
    alarm_level: Mapped[int] = mapped_column(TINYINT, nullable=False, server_default=text("'0'"), comment='报警等级')
    handled_status: Mapped[int] = mapped_column(TINYINT, nullable=False, server_default=text("'0'"), comment='处理状态（0-未处理，1-处理中，2-已处理）')
    alarm_time: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('CURRENT_TIMESTAMP'), comment='报警时间')
    handled_time: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, comment='处理时间')
    handler: Mapped[Optional[str]] = mapped_column(String(50, 'utf8mb4_unicode_ci'), comment='处理人')
    created_time: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('CURRENT_TIMESTAMP'), comment='创建时间')
    updated_time: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'), comment='更新时间')
...
class CuttingStatusTable(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, server_default=text("'1'"), comment='主键ID，仅允许存在一条记录')
    feed_rate: Mapped[Optional[decimal.Decimal]] = mapped_column(DECIMAL(10, 3), nullable=False, server_default=text("'0.000'"), comment='进给速度(mm/s)')
    torque: Mapped[Optional[decimal.Decimal]] = mapped_column(DECIMAL(10, 3), nullable=False, server_default=text("'0.000'"), comment='主轴扭矩(N·m)')
    elapsed_sec: Mapped[Optional[decimal.Decimal]] = mapped_column(DECIMAL(10, 3), nullable=False, server_default=text("'0.000'"), comment='累计执行时长(秒)')
    spindle_rpm: Mapped[Optional[decimal.Decimal]] = mapped_column(DECIMAL(10, 2), nullable=False, server_default=text("'0.00'"), comment='主轴转速(rpm)')
    updated_time: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'), comment='最近更新时间')
    created_time: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('CURRENT_TIMESTAMP'), comment='创建时间')
