...
class ActionTable(Base):
    ...
    priority: Mapped[Optional[int]] = mapped_column(TINYINT, server_default=text("'1'"), comment='优先级（1-低，2-中，3-高）')
    execute_times: Mapped[Optional[int]] = mapped_column(Integer, server_default=text("'0'"), comment='执行次数')
    max_retries: Mapped[Optional[int]] = mapped_column(Integer, server_default=text("'3'"), comment='最大重试次数')
...
class HardwareTaskQueue(Base):
    ...
    priority: Mapped[Optional[int]] = mapped_column(TINYINT, server_default=text("'0'"), comment='优先级：0-普通 1-高 2-紧急')
    status: Mapped[Optional[int]] = mapped_column(TINYINT, server_default=text("'0'"), comment='状态：0-待执行 1-执行中 2-执行成功 3-执行失败 4-已取消')
    retry_count: Mapped[Optional[int]] = mapped_column(TINYINT, server_default=text("'0'"), comment='重试次数')
    max_retry_count: Mapped[Optional[int]] = mapped_column(TINYINT, server_default=text("'3'"), comment='最大重试次数')
    ...
    result_message: Mapped[Optional[str]] = mapped_column(TEXT, comment='执行结果信息')
    created_by: Mapped[Optional[str]] = mapped_column(VARCHAR(50), comment='创建者')
    created_time: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('CURRENT_TIMESTAMP'), comment='创建时间')
    updated_time: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'), comment='更新时间')
...
class MaintenanceTable(Base):
    ...
    m_status: Mapped[Optional[int]] = mapped_column(TINYINT, server_default=text("'0'"), comment='状态（0-待执行，1-执行中，2-已完成）')
...
class WorkpieceTable(Base):
    ...
    w_roughness_required: Mapped[Optional[decimal.Decimal]] = mapped_column(DECIMAL(5, 3), comment='要求粗糙度')
    w_roughness_actual: Mapped[Optional[decimal.Decimal]] = mapped_column(DECIMAL(5, 3), comment='实际粗糙度')
    w_status: Mapped[Optional[int]] = mapped_column(TINYINT, server_default=text("'0'"), comment='状态（0-待加工，1-加工中，2-已完成，3-不合格）')
    w_task_id: Mapped[Optional[int]] = mapped_column(BigInteger, comment='关联任务ID')
    created_time: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('CURRENT_TIMESTAMP'), comment='创建时间')
