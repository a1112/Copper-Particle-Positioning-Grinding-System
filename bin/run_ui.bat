...
REM Switch to script directory (repo root)
cd /d "%~dp0\.."

REM Require python in PATH
...
echo [INFO] Launching UI only (%_PY% -m app.ui.run_ui)...
if not exist "logs" mkdir "logs"
set "_LOG=logs\ui_run.log"
echo ==== %DATE% %TIME% ==== > "%_LOG%"
"%_PY%" -m app.ui.run_ui 1>>"%_LOG%" 2>&1
